import { Skeleton, Grid, ListItemIcon } from '@mui/material';
...
const TravelMode = ({ gMapsApiStatus, setMode, className }: Props) => {
    ...
    return (
        <Grid item xs={12} className={className}>
            {gMapsApiStatus && travel.length > 0 ? (
                <div className="flex w-full justify-center border-b border-gray-500 pb-1 gap-8">
                    {travel.map((mode, index) => (
                        <button
                            key={index}
                            type="button"
                            className={`btn cursor-pointer w-8 ${selectedMode === mode.mode ? 'bg-[#a9c2df] text-white shadow-md' : 'bg-gray-400 hover:bg-[#a9c2df] focus:outline-none focus:ring-0 focus:border-0 border-0'} rounded-full`}
                            onClick={() => onSelectTravel(mode.name)} // Pass the mode name to the function
                        >
                            <ListItemIcon className="min-w-0 flex justify-center">
